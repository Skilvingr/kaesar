...
use kaesar_core::view::handle_event;
...
use kaesar_core::view::renderer::{
    RenderData, RenderQueue, UpdateData, process_render_queue, wait_for_all,
};
...
use kaesar_core::view::{AppCmd, EntryId, EntryKind, Event, View, ViewId};
...
fn resume(
    ...
    if id == TaskId::Suspend || id == TaskId::PrepareSuspend {
        ...
        if let Some(index) = locate::<Intermission>(view) {
            ...
            RenderQueue::add_redraw_req(rendering_ctx, RenderData::expose(rect, UpdateMode::Full));
        }
...
fn power_off_or_reboot(
    ...
    view.handle_event(&Event::Back, &tx, &mut VecDeque::new(), &mut None, context);
    while let Some(mut item) = history.pop() {
        item.view
            .handle_event(&Event::Back, &tx, &mut VecDeque::new(), &mut None, context);
    }
    ...
    wait_for_all(context.fb.as_mut(), updating);
    interm.render_view(interm.rect(), context);
...
pub fn run(
    ...
    let mut rendering_ctx = Some(RenderQueue::default());
    ...
    //     Box::new(TouchEvents::new(context.fb.rect(), &mut ctx, &mut context));
        ...
        match evt {
            Event::Device(de) => match de {
                DeviceEvent::Button {
                    ...
                    if tasks.iter().any(|task| task.id == TaskId::PrepareSuspend) {
                        ...
                        let interm =
                            Intermission::new(context.rect(), IntermKind::Suspend, &context);

                        let (id, rect) = (interm.id(), *interm.rect());
                        view.children_mut().push(Box::new(interm) as Box<dyn View>);

                        RenderQueue::add_redraw_req(
                            &mut rendering_ctx,
                            RenderData::new(id, rect, UpdateMode::Full),
                        );
                ...
                DeviceEvent::CoverOn => {
                    ...
                    let interm = Intermission::new(context.rect(), IntermKind::Suspend, &context);
                    ...
                    match power_source {
                        ...
                        PowerSource::Host => {
                            ...
                            if context.settings.auto_share {
                                ...
                                let (id, rect) = (dialog.id(), *dialog.rect());
                                view.children_mut().push(Box::new(dialog) as Box<dyn View>);

                                RenderQueue::add_redraw_req(
                                    &mut rendering_ctx,
                                    RenderData::new(id, rect, UpdateMode::Gui),
                                );
                            }
                ...
                DeviceEvent::Unplug(..) => {
                    ...
                    if context.shared {
                        ...
                        if let Some(index) = locate::<Intermission>(view.as_ref()) {
                            ...
                            RenderQueue::add_redraw_req(
                                &mut rendering_ctx,
                                RenderData::expose(rect, UpdateMode::Full),
                            );
                        }
            ...
            Event::BatteryTick => {
                ...
                if let Ok(capacity) = context.battery.capacity().map(|v| v[0]) {
                    ...
                    {
                        top_bar.reseed(Some(capacity), None, &mut rendering_ctx, &mut context);
                    }
                }
            }
            Event::PrepareSuspend => {
                tasks.retain(|task| task.id != TaskId::PrepareSuspend);
                wait_for_all(context.fb.as_mut(), &mut updating);
            ...
            Event::PrepareShare => {
                ...
                let interm = Intermission::new(context.rect(), IntermKind::Share, &context);
                let (id, rect) = (interm.id(), *interm.rect());
                view.children_mut().push(Box::new(interm) as Box<dyn View>);
                    ...
                    GestureEvent::MultiTap(points) => {
                        ...
                        match (r1, r2) {
                            (
                                ...
                                RenderQueue::add_redraw_req(
                                    &mut rendering_ctx,
                                    RenderData::new(view.id(), context.rect(), UpdateMode::Full),
                                );
                            }
                            (
            ...
            Event::Open(info) => {
                ...
                if let Some(r) = Reader::new(context.rect(), *info, &kaesar_ev_tx, &mut context) {
            ...
            Event::Select(EntryId::SystemInfo) => {
                ...
                let r = Reader::from_html(context.rect(), &html, None, &kaesar_ev_tx, &mut context);
                let mut next_view = Box::new(r) as Box<dyn View>;
                transfer_notifications(
                    view.as_mut(),
                    next_view.as_mut(),
            ...
            Event::TogglePresetMenu(rect, index) => {
                if let Some(index) = locate_by_id(view.as_ref(), ViewId::PresetMenu) {
                    ...
                    let (id, rect) = (preset_menu.id(), *preset_menu.rect());
                ...
                if n != context.display.rotation && view.might_rotate() =>
            {
                ...
                if let Ok(dims) = context.fb.set_rotation(n) {
                    ...
                    if context.display.dims != dims {
                        context.display.dims = dims;
                        view.resize(fb_rect, &kaesar_ev_tx, &mut rendering_ctx, &mut context);
                    } else {
                        RenderQueue::add_redraw_req(
                            &mut rendering_ctx,
                            RenderData::new(view.id(), context.rect(), UpdateMode::Full),
            ...
            Event::Select(EntryId::TakeScreenshot) => {
                ...
                let notif =
                    Notification::new(msg, None, &kaesar_ev_tx, &mut rendering_ctx, &mut context);
                view.children_mut().push(Box::new(notif) as Box<dyn View>);
            }
            Event::CheckFetcher(..)
    ...
    let path = Path::new(SETTINGS_PATH);
    if let Err(e) = save_toml(&context.settings, path) {
        eprintln!("Can't save settings: {e}");
    }
