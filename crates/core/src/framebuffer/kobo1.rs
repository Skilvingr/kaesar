...
use crate::framebuffer::ColourSpace;
...
impl Framebuffer for KoboFramebuffer1 {
    fn colour_space(&self) -> ColourSpace {
        let rgb_not_bgr = self.red_index < self.blue_index;

        match self.var_info.bits_per_pixel {
            32 if rgb_not_bgr => ColourSpace::Rgba,
            32 if !rgb_not_bgr => ColourSpace::Bgra,
            24 if rgb_not_bgr => ColourSpace::Rgb,
            24 if !rgb_not_bgr => ColourSpace::Bgr,
            16 => ColourSpace::High16,
            8 => ColourSpace::Greyscale,
            n => {
                eprintln!("Unrecognised colour depth: {n}. Continuing with greyscale.");
...
fn set_pixel_rgb_32(fb: &mut KoboFramebuffer1, x: u32, y: u32, rgb: [u8; 3]) {
    ...
    unsafe {
        let spot = fb.frame.offset(addr) as *mut u8;
        spot.copy_from_nonoverlapping(rgb.as_ptr(), 3);
        // *spot.offset(3) = 0xFF;
    }
}
