...
use crate::view::{
    Bus, Event, Hub, ID_FEEDER, Id, RenderData, SliderId, THICKNESS_MEDIUM, View, ViewId,
};
...
impl ToolBar {
    ...
    pub fn update_margin_width(
        ...
        if let Some(labeled_icon) = self.children[index].downcast_mut::<LabeledIcon>() {
            labeled_icon.update(&format!("{} mm", margin_width), rendering_ctx);
        }
    }

    pub fn update_font_family(
        &mut self,
        font_family: String,
        rendering_ctx: &mut Option<RenderQueue>,
    ) {
        if let Some(labeled_icon) = self.children[1].downcast_mut::<LabeledIcon>() {
            labeled_icon.update(&font_family, rendering_ctx);
        }
    }
    ...
    pub fn update_line_height(
        ...
        if let Some(labeled_icon) = self.children[2].downcast_mut::<LabeledIcon>() {
            labeled_icon.update(&format!("{:.1} em", line_height), rendering_ctx);
        }
    }

    pub fn update_text_align_icon(
        &mut self,
        ...
        if icon.name != name {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(icon.id(), *icon.rect(), UpdateMode::Gui),
            );
        }
    }

    pub fn update_font_size_slider(
        &mut self,
        font_size: f32,
        rendering_ctx: &mut Option<RenderQueue>,
    ) {
        let slider = self.children[6].as_mut().downcast_mut::<Slider>().unwrap();
        slider.update(font_size, rendering_ctx);
    }

    pub fn update_contrast_exponent_slider(
        &mut self,
        exponent: f32,
        rendering_ctx: &mut Option<RenderQueue>,
    ) {
        let slider = self.children[1].as_mut().downcast_mut::<Slider>().unwrap();
        slider.update(exponent, rendering_ctx);
    }

    pub fn update_contrast_grey_slider(
        &mut self,
        grey: f32,
        rendering_ctx: &mut Option<RenderQueue>,
    ) {
        let slider = self.children[3].as_mut().downcast_mut::<Slider>().unwrap();
        slider.update(grey, rendering_ctx);
    }
}
...
impl View for ToolBar {
    ...
    fn resize(
        ...
        if self.reflowable {
            ...
            let text_align_rect =
                rect![rect.min.x, rect.max.y - side, rect.min.x + side, rect.max.y];
            self.children[index].resize(text_align_rect, hub, rendering_ctx, context);
            index += 1;
            ...
            self.children[index].resize(font_size_rect, hub, rendering_ctx, context);
            index += 1;
            ...
            self.children[index].resize(contrast_icon_rect, hub, rendering_ctx, context);
            index += 1;
            ...
            self.children[index].resize(grey_icon_rect, hub, rendering_ctx, context);
            index += 1;
