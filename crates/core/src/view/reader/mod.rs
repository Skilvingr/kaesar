...
use crate::view::{AppCmd, Bus, Event, Hub, RenderData, View};
...
impl Reader {
    ...
    fn go_to_page(
        ...
        if let Some(location) = loc {
            ...
            self.update(None, hub, rendering_ctx, context);
            self.update_bottom_bar(rendering_ctx);

            if self.search.is_some() {
                self.update_results_bar(rendering_ctx);
            }
        }
    }
    ...
    fn go_to_chapter(
        ...
        if let Some(location) = loc {
            self.go_to_page(location, true, hub, rendering_ctx, context);
        }
    }
    ...
    fn go_to_last_page(
        ...
        if let Some(location) = self.history.pop_back() {
            self.go_to_page(location, false, hub, rendering_ctx, context);
        }
    }
    ...
    fn _directional_scroll(
        ...
        if vprect.overlaps(&frame) {
            ...
            self.update(update_mode, hub, rendering_ctx, context);
        }
    }
    ...
    fn directional_scroll(
        ...
        self._directional_scroll(delta, hub, rendering_ctx, context, None);
    }
    ...
    fn directional_scroll_fast(
        ...
        self._directional_scroll(
            delta,
            hub,
            rendering_ctx,
            context,
            Some(UpdateMode::FastMono),
        );
    }
    ...
    fn update_bottom_bar(&mut self, rendering_ctx: &mut Option<RenderQueue>) {
        if let Some(index) = locate::<BottomBar>(self) {
            ...
            bottom_bar.update_chapter_label(title, progress, rendering_ctx);
            bottom_bar.update_page_label(self.current_page, self.pages_count, rendering_ctx);
            bottom_bar.update_icons(&neighbors, rendering_ctx);
        }
    }

    fn update_tool_bar(&mut self, rendering_ctx: &mut Option<RenderQueue>, context: &mut Context) {
        if let Some(index) = locate::<ToolBar>(self) {
            ...
            if self.reflowable {
                ...
                tool_bar.update_font_family(font_family, rendering_ctx);
                let font_size = self
                    .info
                    .reader
                    .as_ref()
                    .and_then(|r| r.font_size)
                    .unwrap_or(settings.reader.font_size);
                tool_bar.update_font_size_slider(font_size, rendering_ctx);
                let text_align = self
                    .info
                    .reader
                    .as_ref()
                    .and_then(|r| r.text_align)
                    .unwrap_or(settings.reader.text_align);
                tool_bar.update_text_align_icon(text_align, rendering_ctx);
                let line_height = self
                    ...
                    .and_then(|r| r.line_height)
                    .unwrap_or(settings.reader.line_height);
                tool_bar.update_line_height(line_height, rendering_ctx);
            } else {
                tool_bar.update_contrast_exponent_slider(self.contrast.exponent, rendering_ctx);
                tool_bar.update_contrast_grey_slider(self.contrast.grey, rendering_ctx);
            }
            ...
            tool_bar.update_margin_width(margin_width, rendering_ctx);
        }
    }

    fn update_results_bar(&mut self, rendering_ctx: &mut Option<RenderQueue>) {
        ...
        if let Some(index) = locate::<ResultsBar>(self) {
            ...
            results_bar.update_results_label(count, rendering_ctx);
            results_bar.update_page_label(current_page, pages_count, rendering_ctx);
            results_bar.update_icons(current_page, pages_count, rendering_ctx);
        }
    }
    ...
    fn update(
        ...
        RenderQueue::add_redraw_req(
            rendering_ctx,
            RenderData::no_wait(self.id, self.rect, update_mode),
        );
    ...
    fn search(
        ...
        if self.search.is_some() {
            self.render_results(rendering_ctx);
        }
    ...
    fn toggle_keyboard(
        ...
        if let Some(index) = locate::<Keyboard>(self) {
            ...
            if index == 1 {
                ...
                RenderQueue::add_redraw_req(
                    rendering_ctx,
                    RenderData::expose(rect, UpdateMode::Gui),
                );
            } else {
                ...
                for i in start_index..index - 1 {
                    ...
                    self.child_mut(i)
                        .resize(shifted_rect, hub, rendering_ctx, context);
                    RenderQueue::add_redraw_req(
                        rendering_ctx,
                        RenderData::new(self.child(i).id(), shifted_rect, UpdateMode::Gui),
                    );
                }
    ...
    fn toggle_results_bar(
        ...
        if let Some(index) = locate::<ResultsBar>(self) {
            ...
            if let Some(ref s) = self.search {
                ...
                RenderQueue::add_redraw_req(
                    rendering_ctx,
                    RenderData::new(self.id, rect, UpdateMode::Gui),
                );
            }
        }
    }
    ...
    fn toggle_search_bar(
        ...
        if let Some(index) = locate::<SearchBar>(self) {
            ...
            if self.child(0).is::<TopBar>() {
                self.toggle_bars(Some(false), hub, rendering_ctx, context);
            } else {
                let mut rect = *self.child(index).rect();
                rect.absorb(self.child(index - 1).rect());
                rect.absorb(self.child(index + 1).rect());
            ...
            self.toggle_tool_bar(false, rendering_ctx, context);
    ...
    fn toggle_bars(
        ...
        if let Some(top_index) = locate::<TopBar>(self) {
            ...
            if let Some(bottom_index) = locate::<BottomBar>(self) {
                ...
                RenderQueue::add_redraw_req(
                    rendering_ctx,
                    RenderData::expose(top_rect, UpdateMode::Gui),
                );
                RenderQueue::add_redraw_req(
                    rendering_ctx,
                    RenderData::expose(bottom_rect, UpdateMode::Gui),
                );
                hub.send(Event::Focus(None)).ok();
            }
        } else {
    ...
    fn toggle_margin_cropper(
        ...
        if let Some(index) = locate::<MarginCropper>(self) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(margin_cropper.id(), *margin_cropper.rect(), UpdateMode::Gui),
            );
            self.children
    ...
    fn toggle_edit_note(
        ...
        if let Some(index) = locate_by_id(self, ViewId::EditNote) {
            ...
            if let Some(text) = text.as_ref() {
                edit_note.set_text(text, &mut None, context);
            }

            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(edit_note.id(), *edit_note.rect(), UpdateMode::Gui),
            );
    ...
    fn toggle_name_page(
        ...
        if let Some(index) = locate_by_id(self, ViewId::NamePage) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(name_page.id(), *name_page.rect(), UpdateMode::Gui),
            );
    ...
    fn toggle_go_to_page(
        ...
        if let Some(index) = locate_by_id(self, id) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(go_to_page.id(), *go_to_page.rect(), UpdateMode::Gui),
            );
    ...
    pub fn toggle_annotation_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::AnnotationMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(selection_menu.id(), *selection_menu.rect(), UpdateMode::Gui),
            );
            self.children
    ...
    pub fn toggle_title_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::TitleMenu) {
            ...
            entries.push(EntryKind::CheckBox(
                "Invert Document".to_string(),
                EntryId::ToggleDocumentInverted,
                self.info
                    .reader
                    .as_ref()
                    .map(|i| i.inverted)
                    .unwrap_or(context.fb.inverted()),
            ));
            ...
            title_menu
                ...
                .unwrap()
                .set_disabled(zoom_mode != ZoomMode::FitToWidth, rendering_ctx);
    ...
    fn toggle_font_size_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::FontSizeMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(font_size_menu.id(), *font_size_menu.rect(), UpdateMode::Gui),
            );
            self.children
    ...
    fn toggle_page_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::PageMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(page_menu.id(), *page_menu.rect(), UpdateMode::Gui),
            );
    ...
    fn set_font_size(
        ...
        self.text.clear();
        self.update(None, hub, rendering_ctx, context);
        self.update_tool_bar(rendering_ctx, context);
        self.update_bottom_bar(rendering_ctx);
    }
    ...
    fn toggle_bookmark(&mut self, rendering_ctx: &mut Option<RenderQueue>) {
        if let Some(ref mut r) = self.info.reader {
    ...
    fn render_results(&self, rendering_ctx: &mut Option<RenderQueue>) {
        for chunk in &self.chunks {
    ...
    fn quit(&mut self, context: &mut Context) {
        ...
        if let Some(ref mut r) = self.info.reader {
            ...
            r.rotation = Some(CURRENT_DEVICE.to_canonical(context.fb.rotation()));
...
impl View for Reader {
    fn handle_event(
        ...
        match *evt {
            Event::Gesture(GestureEvent::Rotate { quarter_turns, .. }) if quarter_turns != 0 => {
                ...
                let n = (4 + (context.fb.rotation() - dir * quarter_turns)) % 4;
            ...
            Event::Gesture(GestureEvent::Movement { start, end }) => {
                ...
                if let ZoomMode::Custom(_) = self.view_port.zoom_mode {
                    self.directional_scroll_fast(start - end, hub, rendering_ctx, context);
            ...
            Event::Gesture(GestureEvent::MovementEnded {}) if self.state == State::Panning => {
                ...
                self.update(Some(UpdateMode::Full), hub, rendering_ctx, context);
                true
            }
                ...
                if self.rect.includes(start) =>
            {
                match self.view_port.zoom_mode {
                    ZoomMode::FitToPage | ZoomMode::FitToWidth => {
                        match dir {
                            Dir::West => {
                                self.go_to_neighbor(CycleDir::Next, hub, rendering_ctx, context)
                            }
                            Dir::East => {
                                self.go_to_neighbor(CycleDir::Previous, hub, rendering_ctx, context)
                            }
                            Dir::South | Dir::North => {
                                self.vertical_scroll(start.y - end.y, hub, rendering_ctx, context)
                            }
                        };
                    }
                    _ => {}
            ...
            {
                if let ZoomMode::Custom(_) = self.view_port.zoom_mode {
                    self.directional_scroll(start - end, hub, rendering_ctx, context);
                }
                true
            }
            Event::Gesture(GestureEvent::Spread {
                ...
                if !self.reflowable {
                    self.set_zoom_mode(ZoomMode::FitToWidth, true, hub, rendering_ctx, context);
                }
                true
            }
            Event::Gesture(GestureEvent::Pinch {
                ...
                self.set_zoom_mode(ZoomMode::FitToPage, true, hub, rendering_ctx, context);
                true
            }
            Event::Gesture(GestureEvent::Spread {
                axis: Axis::Vertical,
                ...
                if !self.reflowable {
                    self.set_scroll_mode(ScrollMode::Screen, hub, rendering_ctx, context);
                }
                true
            }
            Event::Gesture(GestureEvent::Pinch {
                ...
                if !self.reflowable {
                    self.set_scroll_mode(ScrollMode::Page, hub, rendering_ctx, context);
                }
                true
            }
            Event::Gesture(GestureEvent::Spread {
            ...
            | Event::Gesture(GestureEvent::Pinch {
                ...
                self.scale_page(center, factor, hub, rendering_ctx, context);
                true
            }
            Event::Gesture(GestureEvent::Arrow { dir, .. }) => {
                match dir {
                    Dir::West => {
                        if self.search.is_none() {
                            self.go_to_chapter(CycleDir::Previous, hub, rendering_ctx, context);
                        } else {
                            self.go_to_results_page(0, hub, rendering_ctx, context);
                        }
                    }
                    Dir::East => {
                        if self.search.is_none() {
                            self.go_to_chapter(CycleDir::Next, hub, rendering_ctx, context);
                        } else {
                            let last_page = self.search.as_ref().unwrap().highlights.len() - 1;
                            self.go_to_results_page(last_page, hub, rendering_ctx, context);
                        }
                    }
                    Dir::North => {
                        self.search_direction = LinearDir::Backward;
                        self.toggle_search_bar(true, hub, rendering_ctx, context);
                    }
                    Dir::South => {
                        self.search_direction = LinearDir::Forward;
                        self.toggle_search_bar(true, hub, rendering_ctx, context);
            ...
            Event::Gesture(GestureEvent::Corner { dir, .. }) => {
                match dir {
                    DiagDir::NorthWest => {
                        self.go_to_bookmark(CycleDir::Previous, hub, rendering_ctx, context)
                    }
                    DiagDir::NorthEast => {
                        self.go_to_bookmark(CycleDir::Next, hub, rendering_ctx, context)
                    }
            ...
            Event::Gesture(GestureEvent::MultiCorner { dir, .. }) => {
                match dir {
                    DiagDir::NorthWest => {
                        self.go_to_annotation(CycleDir::Previous, hub, rendering_ctx, context)
                    }
                    DiagDir::NorthEast => {
                        self.go_to_annotation(CycleDir::Next, hub, rendering_ctx, context)
                    }
                    _ => (),
                }
                true
            ...
            Event::Gesture(GestureEvent::Diamond(_)) => {
                self.toggle_bars(None, hub, rendering_ctx, context);
                true
            }
                        ...
                        if let Some(mut i) = rects.iter().position(|(_, loc)| *loc == start_low) {
                            ...
                            while rects[i].1 < start_high {
                                    ...
                                    > rect.height().min(next_rect.height()) as i32 / 2
                                {
                                    ...
                                    RenderQueue::add_redraw_req(
                                        rendering_ctx,
                                        RenderData::new(self.id, rect, UpdateMode::Fast),
                                    );
                                    rect = next_rect;
                                }
                                i += 1;
            ...
            Event::Gesture(GestureEvent::Tap(center)) if self.rect.includes(center) => {
                ...
                if let Some(link) = nearest_link.take() {
                    ...
                    if let Some(caps) = toc_page.captures(&link.text) {
                        ...
                        if let Ok(mut index) = caps[2].parse::<usize>() {
                            ...
                            self.go_to_page(index, true, hub, rendering_ctx, context);
                        }
                    } else {
                        let mut doc = self.doc.lock().unwrap();
                ...
                if let ZoomMode::Custom(_) = self.view_port.zoom_mode {
                    ...
                    match Region::from_point(
                        ...
                        Region::Corner(diag_dir) => match diag_dir {
                            DiagDir::NorthEast => {
                                self.directional_scroll(pt!(dx, -dy), hub, rendering_ctx, context)
                            }
                            DiagDir::SouthEast => {
                                self.directional_scroll(pt!(dx, dy), hub, rendering_ctx, context)
                            }
                        ...
                        Region::Strip(dir) => match dir {
                            Dir::North => {
                                self.directional_scroll(pt!(0, -dy), hub, rendering_ctx, context)
                            }
                            Dir::East => {
                                self.directional_scroll(pt!(dx, 0), hub, rendering_ctx, context)
                            }
                            Dir::South => {
                                self.directional_scroll(pt!(0, dy), hub, rendering_ctx, context)
                            }
                        ...
                        Region::Center => self.toggle_bars(None, hub, rendering_ctx, context),
                    }
                ...
                match Region::from_point(
                    ...
                    Region::Corner(diag_dir) => match diag_dir {
                        DiagDir::NorthWest => self.go_to_last_page(hub, rendering_ctx, context),
                        DiagDir::NorthEast => self.toggle_bookmark(rendering_ctx),
                        DiagDir::SouthEast => {
                            if self.search.is_none() {
                    ...
                    Region::Strip(dir) => match dir {
                        ...
                        Dir::North => self.toggle_bars(None, hub, rendering_ctx, context),
                    },
                    Region::Center => self.toggle_bars(None, hub, rendering_ctx, context),
                }
            ...
            Event::Update(mode) => {
                self.update(Some(mode), hub, rendering_ctx, context);
                true
            }
                ...
                if let Some(caps) = re.captures(text) {
                    ...
                    if prefix == Some("'") {
                        ...
                        if text == "_" {
                            ...
                            self.go_to_page(0, true, hub, rendering_ctx, context);
                        } else if text == ")" {
                            self.go_to_page(
            ...
            Event::Submit(ViewId::GoToResultsPageInput, ref text) => {
                if let Ok(index) = text.parse::<usize>() {
                    self.go_to_results_page(index.saturating_sub(1), hub, rendering_ctx, context);
                }
                true
            }
            ...
            Event::Submit(ViewId::ReaderSearchInput, ref text) => {
                match make_query(text) {
                    Some(query) => {
                        self.search(text, query, hub, rendering_ctx);
                        self.toggle_keyboard(false, None, hub, rendering_ctx, context);
                        self.toggle_results_bar(true, rendering_ctx, context);
                    }
                    None => {
                        let notif = Notification::new(
            ...
            Event::Page(dir) => {
                self.go_to_neighbor(dir, hub, rendering_ctx, context);
                true
            }
            ...
            Event::GoToLocation(ref location) => {
                ...
                if let Some(offset) = offset_opt {
                    self.go_to_page(offset, true, hub, rendering_ctx, context);
                }
                true
            }
            Event::Chapter(dir) => {
                self.go_to_chapter(dir, hub, rendering_ctx, context);
                true
            }
            Event::ResultsPage(dir) => {
                self.go_to_results_neighbor(dir, hub, rendering_ctx, context);
                true
            }
            Event::CropMargins(ref margin) => {
                let current_page = self.current_page;
                self.crop_margins(current_page, margin.as_ref(), hub, rendering_ctx, context);
                true
            }
            ...
            Event::Toggle(ViewId::GoToPage) => {
                self.toggle_go_to_page(None, ViewId::GoToPage, hub, rendering_ctx, context);
                true
            }
            Event::Toggle(ViewId::GoToResultsPage) => {
                self.toggle_go_to_page(None, ViewId::GoToResultsPage, hub, rendering_ctx, context);
                true
            }
            Event::Slider(SliderId::FontSize, font_size, FingerStatus::Up) => {
                self.set_font_size(font_size, hub, rendering_ctx, context);
                true
            }
            Event::Slider(SliderId::ContrastExponent, exponent, FingerStatus::Up) => {
                self.set_contrast_exponent(exponent, hub, rendering_ctx, context);
                true
            }
            Event::Slider(SliderId::ContrastGrey, grey, FingerStatus::Up) => {
                self.set_contrast_grey(grey, hub, rendering_ctx, context);
                true
            }
            ...
            Event::ToggleNear(ViewId::MarginCropperMenu, rect) => {
                self.toggle_margin_cropper_menu(rect, None, rendering_ctx, context);
                true
            }
            ...
            Event::ToggleNear(ViewId::FontFamilyMenu, rect) => {
                self.toggle_font_family_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::FontSizeMenu, rect) => {
                self.toggle_font_size_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::TextAlignMenu, rect) => {
                self.toggle_text_align_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::MarginWidthMenu, rect) => {
                self.toggle_margin_width_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::LineHeightMenu, rect) => {
                self.toggle_line_height_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::ContrastExponentMenu, rect) => {
                self.toggle_contrast_exponent_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::ContrastGreyMenu, rect) => {
                self.toggle_contrast_grey_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::PageMenu, rect) => {
                self.toggle_page_menu(rect, None, rendering_ctx, context);
                true
            }
            ...
            Event::Close(ViewId::SearchBar) => {
                self.toggle_results_bar(false, rendering_ctx, context);
                self.toggle_search_bar(false, hub, rendering_ctx, context);
                if let Some(ref mut s) = self.search {
                    s.running.store(false, AtomicOrdering::Relaxed);
                    self.render_results(rendering_ctx);
                    self.search = None;
            ...
            Event::Close(ViewId::GoToPage) => {
                self.toggle_go_to_page(Some(false), ViewId::GoToPage, hub, rendering_ctx, context);
                true
            }
            Event::Close(ViewId::GoToResultsPage) => {
                self.toggle_go_to_page(
                    Some(false),
                    ViewId::GoToResultsPage,
                    hub,
                    rendering_ctx,
                    context,
                );
            ...
            Event::Close(ViewId::EditNote) => {
                self.toggle_edit_note(None, Some(false), hub, rendering_ctx, context);
                if let Some(rect) = self.selection_rect() {
                    self.selection = None;
                    RenderQueue::add_redraw_req(
                        rendering_ctx,
            ...
            Event::Show(ViewId::MarginCropper) => {
                self.toggle_margin_cropper(true, hub, rendering_ctx, context);
                true
            }
            Event::Close(ViewId::MarginCropper) => {
                self.toggle_margin_cropper(false, hub, rendering_ctx, context);
                true
            }
            ...
            Event::Select(EntryId::AnnotateSelection) => {
                self.toggle_edit_note(None, Some(true), hub, rendering_ctx, context);
                true
            }
            ...
            Event::Select(EntryId::GoToSelectedPageName) => {
                if let Some(loc) = self.selected_text().and_then(|text| {
                    ...
                    self.go_to_page(loc, true, hub, rendering_ctx, context);
                }
                if let Some(rect) = self.selection_rect() {
                    RenderQueue::add_redraw_req(
                        rendering_ctx,
            ...
            Event::Select(EntryId::EditAnnotationNote(sel)) => {
                ...
                self.toggle_edit_note(text, Some(true), hub, rendering_ctx, context);
            ...
            Event::Select(EntryId::SetZoomMode(zoom_mode)) => {
                self.set_zoom_mode(zoom_mode, true, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SetScrollMode(scroll_mode)) => {
                self.set_scroll_mode(scroll_mode, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::ToggleDocumentInverted) => {
                if let Some(i) = &mut self.info.reader {
            ...
            Event::Select(EntryId::Save) => {
                ...
                let notif = Notification::new(msg, None, hub, rendering_ctx, context);
                self.children.push(Box::new(notif) as Box<dyn View>);
                true
            }
            ...
            Event::Select(EntryId::SetFontFamily(ref font_family)) => {
                self.set_font_family(font_family, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SetTextAlign(text_align)) => {
                self.set_text_align(text_align, hub, rendering_ctx, context);
                true
            }
            ...
            Event::Select(EntryId::SetMarginWidth(width)) => {
                self.set_margin_width(width, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SetLineHeight(v)) => {
                let line_height = 1.0 + v as f32 / 10.0;
                self.set_line_height(line_height, hub, rendering_ctx, context);
                true
            }
            ...
            Event::Select(EntryId::SetPageName) => {
                self.toggle_name_page(None, hub, rendering_ctx, context);
                true
            }
            ...
            Event::ToggleFrontlight => {
                if let Some(index) = locate::<TopBar>(self) {
                    self.child_mut(index)
                        .downcast_mut::<TopBar>()
                        .unwrap()
                        .update_frontlight_icon(rendering_ctx, context);
                }
                true
            }
            Event::Device(DeviceEvent::Button {
    ...
    fn render_view(&self, rect: &Rectangle, ctx: &mut Context) {
        ctx.fb.draw_rectangle(rect, WHITE);
        ...
        for chunk in &self.chunks {
            ...
            if let Some(region_rect) = rect.intersection(&chunk_rect) {
                ...
                ctx.fb.draw_framed_pixmap_contrast(
                    pixmap,
                    &chunk_frame,
                    chunk_position,
                ...
                if let Some(rects) = self.noninverted_regions.get(&chunk.location) {
                    for r in rects {
                        ...
                        if let Some(ref image_rect) = rect.intersection(&region_rect) {
                            ctx.fb.invert_region(image_rect);
                        }
                    }
                }
                    ...
                    for rects in groups {
                        ...
                        for r in rects {
                            ...
                            if let Some(ref search_rect) = rect.intersection(&region_rect) {
                                ctx.fb.invert_region(search_rect);
                            }
                        ...
                        if let Some(text) = self.text.get(&chunk.location) {
                                ...
                                .filter(|w| w.location >= start && w.location <= end)
                            {
                                ...
                                if let Some(ref sel_rect) = rect.intersection(&region_rect) {
                                    ctx.fb.shift_region(sel_rect, drift);
                                }
                                if let Some(last) = last_rect {
    ...
    fn resize(
        ...
        if !self.children.is_empty() {
            ...
            if self.children[0].is::<TopBar>() {
                ...
                self.children[0].resize(top_bar_rect, hub, rendering_ctx, context);
                ...
                self.children[1].resize(separator_rect, hub, rendering_ctx, context);
                ...
                if self.children[index].is::<SearchBar>() {
                    ...
                    self.children[index].resize(sb_rect, hub, rendering_ctx, context);
                ...
                if self.children[index].is::<Keyboard>() {
                    ...
                    self.children[index].resize(kb_rect, hub, rendering_ctx, context);
                    self.children[index + 1].resize(
                        rect![
                            rect.min.x,
                            kb_rect.max.y,
            ...
            if let Some(mut index) = locate::<BottomBar>(self) {
                ...
                self.children[index - 1].resize(separator_rect, hub, rendering_ctx, context);
                ...
                self.children[index].resize(bottom_bar_rect, hub, rendering_ctx, context);
                ...
                while index > 2 {
                    ...
                    self.children[index].resize(bar_rect, hub, rendering_ctx, context);
                    let y_max = self.children[index].rect().min.y;
                    let sp_rect = rect![rect.min.x, y_max - thickness, rect.max.x, y_max];
                    self.children[index - 1].resize(sp_rect, hub, rendering_ctx, context);
