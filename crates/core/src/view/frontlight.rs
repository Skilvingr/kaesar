...
use super::{Align, Bus, EntryId, Event, Hub, ID_FEEDER, Id, RenderData, SliderId, View, ViewId};
...
impl FrontlightWindow {
    pub fn new(context: &mut Context) -> FrontlightWindow {
        ...
        if !presets.is_empty() {
            ...
            presets_list.update(presets, &mut None, &mut context.fonts);
    ...
    fn toggle_presets(
        ...
        if enable {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::expose(self.rect, UpdateMode::Gui),
            );
    ...
    fn set_frontlight_levels(
        ...
        if CURRENT_DEVICE.has_natural_light() {
            if let Some(slider_intensity) = self.child_mut(3).downcast_mut::<Slider>() {
                slider_intensity.update(intensity, rendering_ctx);
            }
            if let Some(slider_warmth) = self.child_mut(5).downcast_mut::<Slider>() {
                slider_warmth.update(warmth, rendering_ctx);
            }
    ...
    fn update_presets(&mut self, rendering_ctx: &mut Option<RenderQueue>, context: &mut Context) {
        let len = self.len();
...
impl View for FrontlightWindow {
        ...
        match *evt {
            ...
            Event::Save => {
                ...
                if context.settings.frontlight_presets.len() == 1 {
                    self.toggle_presets(true, rendering_ctx, context);
                } else {
                    ...
                    self.update_presets(rendering_ctx, context);
                }
                true
            }
            Event::Select(EntryId::RemovePreset(index)) => {
                if index < context.settings.frontlight_presets.len() {
                    ...
                    if context.settings.frontlight_presets.is_empty() {
                        self.toggle_presets(false, rendering_ctx, context);
                    } else {
                        if context.settings.frontlight_presets.len() == 1 {
                            let index = self.len() - 2;
            ...
            Event::LoadPreset(index) => {
                ...
                self.set_frontlight_levels(frontlight_levels, rendering_ctx, context);
                true
            }
            Event::Guess => {
                let lightsensor_level = if CURRENT_DEVICE.has_lightsensor() {
    ...
    fn resize(
        ...
        if !context.settings.frontlight_presets.is_empty() {
            ...
            self.children[index].resize(presets_rect, hub, rendering_ctx, context);
        }
    }
