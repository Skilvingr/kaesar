...
use super::{Bus, EntryKind, Event, Hub, ID_FEEDER, Id, RenderData, View};
...
impl MenuEntry {
    ...
    pub fn update(&mut self, value: bool, rendering_ctx: &mut Option<RenderQueue>) {
    ...
    pub fn set_disabled(&mut self, value: bool, rendering_ctx: &mut Option<RenderQueue>) {
...
impl View for MenuEntry {
    ...
    fn render_view(&self, _rect: &Rectangle, ctx: &mut Context) {
        ...
        let font = font_from_style(&mut ctx.fonts, &style, dpi);
        ...
        if let Some(ref cs) = self.corner_spec {
            ctx.fb.draw_rounded_rectangle(&self.rect, &cs, scheme[0]);
        } else {
        ...
        if let Some(pixmap) = ICONS_PIXMAPS.get(icon_name) {
            ...
            ctx.fb.draw_blended_pixmap(pixmap, pt, foreground);
        }
    }
