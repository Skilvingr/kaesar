use super::{Align, Bus, Event, Hub, ID_FEEDER, Id, RenderData, View, ViewId};
...
impl View for Icon {
    ...
    fn render_view(&self, _rect: &Rectangle, ctx: &mut Context) {
        ...
        if let Some(ref cs) = self.corners {
            ctx.fb.draw_rounded_rectangle(&self.rect, cs, background);
        } else {
            ctx.fb.draw_rectangle(&self.rect, background);
        }
