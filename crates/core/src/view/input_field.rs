...
use super::{
    Bus, EntryId, Event, Hub, ID_FEEDER, Id, KeyboardEvent, RenderData, TextKind, View, ViewId,
};
        ...
        match *evt {
            ...
            Event::Select(EntryId::SetInputText(view_id, ref text)) => {
                if self.view_id == view_id {
                    self.set_text(text, true, rendering_ctx, context);
                    if !self.focused {
                        bus.push_back(Event::Submit(self.view_id, self.text.clone()));
                    }
                    true
