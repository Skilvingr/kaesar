...
use crate::metadata::{BookQuery, Info, Metadata, SimpleStatus, SortMethod, sort};
...
impl Home {
    pub fn new(
        ...
        if context.settings.home.navigation_bar {
            ...
            nav_bar.set_path(&current_directory, &dirs, &mut None, context);
    ...
    fn select_directory(
        ...
        if context.settings.home.navigation_bar {
            let nav_bar = self.children[index]
                ...
                .unwrap();
            nav_bar.set_path(&self.current_directory, &dirs, rendering_ctx, context);
        ...
        self.update_shelf(true, hub, rendering_ctx, context);
        self.update_bottom_bar(rendering_ctx, context);
    }
    ...
    fn toggle_select_directory(
        ...
        if self.current_directory.starts_with(path) {
            if let Some(parent) = path.parent() {
                self.select_directory(parent, hub, rendering_ctx, context);
            }
        } else {
            self.select_directory(path, hub, rendering_ctx, context);
        }
    }
    ...
    fn go_to_page(
        ...
        self.update_shelf(false, hub, rendering_ctx, context);
        self.update_bottom_bar(rendering_ctx, context);
    }
    ...
    fn update_top_bar(&mut self, search_visible: bool, rendering_ctx: &mut Option<RenderQueue>) {
        if let Some(index) = locate::<TopBar>(self) {
            ...
            top_bar.update_root_icon(name, rendering_ctx);
            top_bar.update_title_label(&self.sort_method.title(), rendering_ctx);
        }
    }

    fn update_bottom_bar(&mut self, rendering_ctx: &mut Option<RenderQueue>, context: &Context) {
        if let Some(index) = rlocate::<BottomBar>(self) {
            ...
            bottom_bar.update_icons(self.current_page, self.pages_count, rendering_ctx);
        }
    }
            ...
            if enable {
                if has_search_bar {
                    for i in self.shelf_index + 1..=self.shelf_index + 4 {
                        ...
                        RenderQueue::add_redraw_req(
                            rendering_ctx,
                            RenderData::new(self.child(i).id(), *self.child(i).rect(), update_mode),
                        );
                    }
                } else {
                    for i in self.shelf_index + 1..=self.shelf_index + 2 {
        ...
        if let Some(index) = locate::<AddressBar>(self) {
            ...
            *self.children[self.shelf_index - 1].rect_mut() += pt!(0, small_height);
            ...
            if context.settings.home.navigation_bar {
                ...
                // Shrink the nav bar.
                if y_shift < 0 {
                    let y_shift = nav_bar.shrink(y_shift, context);
    ...
    fn toggle_search_bar(
        ...
        if update {
            if !search_visible {
                self.refresh_visibles(false, true, hub, rendering_ctx, context);
            }

            self.update_top_bar(search_visible, rendering_ctx);
            ...
            if search_visible {
                ...
                self.update_shelf(true, hub, &mut None, context);
                RenderQueue::add_redraw_req(
                    rendering_ctx,
                    RenderData::new(self.child(self.shelf_index).id(), rect, UpdateMode::Partial),
                );
                ...
                RenderQueue::add_redraw_req(
                    rendering_ctx,
                    RenderData::expose(rect, UpdateMode::Partial),
                );
            } else {
    ...
    fn toggle_rename_document(
        ...
        if let Some(index) = locate_by_id(self, ViewId::RenameDocument) {
            ...
            {
                ren_doc.set_text(text, rendering_ctx, context);
            }
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(ren_doc.id(), *ren_doc.rect(), UpdateMode::Gui),
            );
    ...
    fn toggle_sort_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::SortMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(sort_menu.id(), *sort_menu.rect(), UpdateMode::Gui),
            );
    ...
    fn toggle_book_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::BookMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(book_menu.id(), *book_menu.rect(), UpdateMode::Gui),
            );
    ...
    fn toggle_library_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::LibraryMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(library_menu.id(), *library_menu.rect(), UpdateMode::Gui),
            );
    ...
    fn add_document(
        ...
        self.sort(false, hub, rendering_ctx, context);
        self.refresh_visibles(true, false, hub, rendering_ctx, context);
    }
    ...
    fn empty_trash(
        ...
        let notif = Notification::new(message, None, hub, rendering_ctx, context);
    ...
    fn set_reverse_order(
        ...
        self.current_page = 0;
        self.sort(true, hub, rendering_ctx, context);
    }
    ...
    fn set_sort_method(
        ...
        if let Some(index) = locate_by_id(self, ViewId::SortMenu) {
            self.child_mut(index)
                ...
                .unwrap()
                .update(sort_method.reverse_order(), rendering_ctx);
        }
    ...
    fn load_library(
        ...
        if self.query.is_some() {
            self.toggle_search_bar(Some(false), false, hub, rendering_ctx, context);
            update_top_bar = true;
        }
        ...
        self.select_directory(&home, hub, rendering_ctx, context);
    }
    ...
    fn reseed(
        ...
        context.library.sort(self.sort_method, self.reverse_order);
        self.refresh_visibles(true, false, hub, &mut None, context);

        if let Some(top_bar) = self.child_mut(0).downcast_mut::<TopBar>() {
            top_bar.reseed(None, None, &mut None, context);
        }
...
impl View for Home {
    fn handle_event(
        ...
        match *evt {
                ...
                match dir {
                            ...
                            && self.children[self.shelf_index].rect().includes(end) =>
                    {
                        if !context.settings.home.navigation_bar {
                            ...
                            self.toggle_address_bar(Some(true), true, hub, rendering_ctx, context);
                        }
                    }
                    Dir::North
                    ...
                    {
                        if context.settings.home.address_bar {
                            self.toggle_address_bar(Some(false), true, hub, rendering_ctx, context);
            ...
            Event::Gesture(GestureEvent::Arrow { dir, .. }) => {
                match dir {
                    Dir::West => self.go_to_page(0, hub, rendering_ctx, context),
                    Dir::East => {
                        let pages_count = self.pages_count;
                        self.go_to_page(pages_count.saturating_sub(1), hub, rendering_ctx, context);
                    }
                    Dir::North => {
                        let path = context.library.home.clone();
                        self.select_directory(&path, hub, rendering_ctx, context);
                    }
                    Dir::South => self.toggle_search_bar(None, true, hub, rendering_ctx, context),
                };
                true
            }
            Event::Gesture(GestureEvent::Corner { dir, .. }) => {
                match dir {
                    DiagDir::NorthWest | DiagDir::SouthWest => {
                        self.go_to_status_change(CycleDir::Previous, hub, rendering_ctx, context)
                    }
                    DiagDir::NorthEast | DiagDir::SouthEast => {
                        self.go_to_status_change(CycleDir::Next, hub, rendering_ctx, context)
                    }
                };
                true
            }
            Event::Focus(v) => {
                if self.focus != v {
                    self.focus = v;
                    if v.is_some() {
                        self.toggle_keyboard(true, true, v, hub, rendering_ctx, context);
                    }
                }
                true
            }
            Event::Show(ViewId::Keyboard) => {
                self.toggle_keyboard(true, true, None, hub, rendering_ctx, context);
                true
            }
            Event::Toggle(ViewId::GoToPage) => {
                self.toggle_go_to_page(None, hub, rendering_ctx, context);
                true
            }
            Event::Toggle(ViewId::SearchBar) => {
                self.toggle_search_bar(None, true, hub, rendering_ctx, context);
                true
            }
            Event::ToggleNear(ViewId::TitleMenu, rect) => {
                self.toggle_sort_menu(rect, None, rendering_ctx, context);
                true
            }
            Event::ToggleBookMenu(rect, index) => {
                self.toggle_book_menu(index, rect, None, rendering_ctx, context);
                true
            }
            ...
            Event::ToggleNear(ViewId::LibraryMenu, rect) => {
                self.toggle_library_menu(rect, None, rendering_ctx, context);
                true
            }
            ...
            Event::Close(ViewId::SearchBar) => {
                self.toggle_search_bar(Some(false), true, hub, rendering_ctx, context);
                true
            }
            Event::Close(ViewId::SortMenu) => {
                self.toggle_sort_menu(Rectangle::default(), Some(false), rendering_ctx, context);
                true
            }
            Event::Close(ViewId::LibraryMenu) => {
                self.toggle_library_menu(Rectangle::default(), Some(false), rendering_ctx, context);
                true
            }
            ...
            Event::Close(ViewId::GoToPage) => {
                self.toggle_go_to_page(Some(false), hub, rendering_ctx, context);
                true
            }
            Event::Close(ViewId::RenameDocument) => {
                self.toggle_rename_document(Some(false), hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::Sort(sort_method)) => {
                let selected_library = context.settings.selected_library;
                context.settings.libraries[selected_library].sort_method = sort_method;
                self.set_sort_method(sort_method, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::ReverseOrder) => {
                let next_value = !self.reverse_order;
                self.set_reverse_order(next_value, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::LoadLibrary(index)) => {
                self.load_library(index, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::Import) => {
                self.import(hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::CleanUp) => {
                self.clean_up(hub, rendering_ctx, context);
                true
            }
            ...
            Event::FetcherAddDocument(_, ref info) => {
                self.add_document(*info.clone(), hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SetStatus(ref path, status)) => {
                self.set_status(path, status, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::FirstColumn(first_column)) => {
                let selected_library = context.settings.selected_library;
                context.settings.libraries[selected_library].first_column = first_column;
                self.update_first_column(hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SecondColumn(second_column)) => {
                let selected_library = context.settings.selected_library;
                context.settings.libraries[selected_library].second_column = second_column;
                self.update_second_column(hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::ThumbnailPreviews) => {
                ...
                context.settings.libraries[selected_library].thumbnail_previews =
                    !context.settings.libraries[selected_library].thumbnail_previews;
                self.update_thumbnail_previews(hub, rendering_ctx, context);
                true
            }
            Event::Submit(ViewId::AddressBarInput, ref addr) => {
                self.toggle_keyboard(false, true, None, hub, rendering_ctx, context);
                self.select_directory(Path::new(addr), hub, rendering_ctx, context);
                true
            }
            Event::Submit(ViewId::HomeSearchInput, ref text) => {
                self.query = BookQuery::new(text);
                if self.query.is_some() {
                    self.toggle_keyboard(false, false, None, hub, rendering_ctx, context);
                    ...
                    self.refresh_visibles(true, true, hub, rendering_ctx, context);
                } else {
            ...
            Event::Submit(ViewId::GoToPageInput, ref text) => {
                if text == "(" {
                    self.go_to_page(0, hub, rendering_ctx, context);
                } else if text == ")" {
                    self.go_to_page(
                    ...
                    self.go_to_page(index, hub, rendering_ctx, context);
                } else if let Ok(index) = text.parse::<usize>() {
                    self.go_to_page(index.saturating_sub(1), hub, rendering_ctx, context);
                }
                true
            }
            Event::Submit(ViewId::RenameDocumentInput, ref file_name) => {
                if let Some(ref path) = self.target_document.take() {
                    self.rename(path, file_name, hub, rendering_ctx, context)
            ...
            Event::Select(EntryId::EmptyTrash) => {
                self.empty_trash(hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::Rename(ref path)) => {
                self.target_document = Some(path.clone());
                self.toggle_rename_document(Some(true), hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::Remove(ref path))
            | Event::FetcherRemoveDocument(_, ref path) => {
                self.remove(path, hub, rendering_ctx, context)
            ...
            Event::Select(EntryId::MoveTo(ref path, index)) => {
                self.move_to(path, index, hub, rendering_ctx, context)
            ...
            | Event::Select(EntryId::ToggleSelectDirectory(ref path)) => {
                self.toggle_select_directory(path, hub, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SearchAuthor(ref author)) => {
                ...
                if query.is_some() {
                    self.query = query;
                    self.toggle_search_bar(Some(true), false, hub, rendering_ctx, context);
                    self.toggle_keyboard(false, false, None, hub, rendering_ctx, context);
                    if let Some(search_bar) =
                        self.children[self.shelf_index + 2].downcast_mut::<SearchBar>()
                    {
                        search_bar.set_text(&text, rendering_ctx, context);
                    }
                    // Render the search bar and its separator.
                    for i in self.shelf_index + 1..=self.shelf_index + 2 {
                        RenderQueue::add_redraw_req(
            ...
            Event::GoTo(location) => {
                self.go_to_page(location as usize, hub, rendering_ctx, context);
                true
            }
            Event::Chapter(dir) => {
                let pages_count = self.pages_count;
                match dir {
                    CycleDir::Previous => self.go_to_page(0, hub, rendering_ctx, context),
                    CycleDir::Next => {
                        self.go_to_page(pages_count.saturating_sub(1), hub, rendering_ctx, context)
                    }
                }
            ...
            Event::Reseed => {
                self.reseed(hub, rendering_ctx, context);
                true
            }
            _ => false,
        }
    ...
    fn resize(
        ...
        if context.settings.home.navigation_bar {
            ...
            nav_bar.set_path(&self.current_directory, &dirs, &mut None, context);
        ...
        self.children[self.shelf_index].resize(shelf_rect, hub, rendering_ctx, context);

        self.update_shelf(true, hub, &mut None, context);
        self.update_bottom_bar(&mut None, context);
