...
use crate::font::font_from_style;
...
use crate::view::{Bus, Event, Hub, ID_FEEDER, Id, RenderData, THICKNESS_SMALL, View};
...
impl View for Book {
    ...
    fn render_view(&self, _rect: &Rectangle, ctx: &mut Context) {
        ...
        ctx.fb.draw_rectangle(&self.rect, scheme[0]);
        ...
        let (x_height, padding, baseline) = {
            let font = font_from_style(&mut ctx.fonts, &MD_TITLE, dpi);
        ...
        if let Some(preview_path) = self.preview_path.as_ref() {
            ...
            if preview_path.exists() {
                    ...
                    })
                {
                    ...
                    ctx.fb.draw_pixmap(&pixmap, pt);

                    if ctx.fb.inverted() {
                        ctx.fb.invert_region(&rect);
                    }
                }
            }
        ...
        // Author
        let author_width = {
            let font = font_from_style(&mut ctx.fonts, &MD_AUTHOR, dpi);
            ...
            if plan.width > width {
                ...
                if available > 3 * padding {
                    ...
                    if leftover > 2 * padding {
                        ...
                        font.render(ctx.fb.as_mut(), scheme[1], &plan2, pt);
                        title_lines += 1;
                    } else {
        ...
        match self.second_column {
            SecondColumn::Year => {
                let font = font_from_style(&mut ctx.fonts, &MD_YEAR, dpi);
        ...
        // File kind
        {
            ...
            let font = font_from_style(&mut ctx.fonts, &MD_KIND, dpi);
        ...
        // File size
        {
            ...
            let font = font_from_style(&mut ctx.fonts, &MD_SIZE, dpi);
