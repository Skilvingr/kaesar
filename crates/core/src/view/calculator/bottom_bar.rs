...
use crate::view::{Bus, Event, Hub, ID_FEEDER, Id, View, ViewId};
...
impl BottomBar {
    ...
    pub fn update_font_size(&mut self, font_size: f32, rendering_ctx: &mut Option<RenderQueue>) {
        if let Some(labeled_icon) = self.children[3].downcast_mut::<LabeledIcon>() {
            labeled_icon.update(&format!("{:.1} pt", font_size), rendering_ctx);
        }
    }
...
impl View for BottomBar {
    ...
    fn resize(
        ...
        self.children[0].resize(filr_rect, hub, rendering_ctx, context);
        x_offset += paddings[0];
        ...
        self.children[1].resize(margin_width_rect, hub, rendering_ctx, context);
        x_offset += labelled_icon_width;

        let filr_rect = rect![x_offset, rect.min.y, x_offset + paddings[1], rect.max.y];
        self.children[2].resize(filr_rect, hub, rendering_ctx, context);
        x_offset += paddings[1];
        ...
        self.children[3].resize(font_size_rect, hub, rendering_ctx, context);
        x_offset += labelled_icon_width;

        let filr_rect = rect![x_offset, rect.min.y, x_offset + paddings[2], rect.max.y];
        self.children[4].resize(filr_rect, hub, rendering_ctx, context);
