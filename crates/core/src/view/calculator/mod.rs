...
impl Calculator {
    ...
    fn history_navigate(
        ...
        if let Some(cursor) = cursor_opt {
            ...
            if let Some(input_bar) = self.children[4].downcast_mut::<InputBar>() {
                input_bar.set_text(line, !honor_prefix, rendering_ctx, context);
            }
            self.history.cursor = cursor;
        }
    }

    fn update_size(&mut self, rendering_ctx: &mut Option<RenderQueue>, context: &mut Context) {
        let dpi = CURRENT_DEVICE.dpi;
        let font = &mut context.fonts.monospace.regular;
        font.set_size((64.0 * self.font_size) as u32, dpi);
        let char_width = font.plan(" ", None, None).width;
        ...
        if let Some(bottom_bar) = self.children[8].downcast_mut::<BottomBar>() {
            bottom_bar.update_font_size(self.font_size, rendering_ctx);
            bottom_bar.update_margin_width(self.margin_width, rendering_ctx);
        }
    }
    ...
    fn set_font_size(
        ...
        self.update_size(rendering_ctx, context);
        self.refresh(context);
    }
...
impl View for Calculator {
    fn handle_event(
        ...
        match *evt {
            Event::Submit(ViewId::CalculatorInput, ref line) => {
                ...
                if let Some(input_bar) = self.children[4].downcast_mut::<InputBar>() {
                    input_bar.set_text("", true, rendering_ctx, context);
                }
            ...
            Event::History(dir, honor_prefix) => {
                self.history_navigate(dir, honor_prefix, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SetFontSize(v)) => {
                let font_size = 6.0 + v as f32 / 10.0;
                self.set_font_size(font_size, rendering_ctx, context);
                true
            }
            Event::Select(EntryId::SetMarginWidth(width)) => {
                self.set_margin_width(width, rendering_ctx, context);
                true
            }
    ...
    fn resize(
        ...
        self.children[6].resize(kb_rect, hub, rendering_ctx, context);
        ...
        self.children[2].resize(code_area_rect, hub, rendering_ctx, context);
        self.children[3].resize(sp_rect2, hub, rendering_ctx, context);
        self.children[4].resize(input_bar_rect, hub, rendering_ctx, context);
        self.children[5].resize(sp_rect, hub, rendering_ctx, context);
        ...
        self.children[7].resize(sp_rect, hub, rendering_ctx, context);
        ...
        self.children[8].resize(bottom_bar_rect, hub, rendering_ctx, context);

        for i in 9..self.children.len() {
            self.children[i].resize(rect, hub, rendering_ctx, context);
        }

        self.update_size(&mut None, context);
        self.refresh(context);
