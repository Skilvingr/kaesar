...
impl Sketch {
    ...
    fn toggle_title_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::SketchMenu) {
            ...
            RenderQueue::add_redraw_req(
                rendering_ctx,
                RenderData::new(sketch_menu.id(), *sketch_menu.rect(), UpdateMode::Gui),
            );
...
fn draw_segment(
    ...
    if let Some(render_rect) = rect.intersection(fb_rect) {
        RenderQueue::add_redraw_req(
            rendering_ctx,
            RenderData::no_wait(id, render_rect, UpdateMode::FastMono),
        );
    }
...
impl View for Sketch {
    ...
    fn render_view(&self, rect: &Rectangle, ctx: &mut Context) {
        ctx.fb
            .draw_framed_pixmap_halftone(&self.pixmap, rect, rect.min);
    }
