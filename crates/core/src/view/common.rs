...
use super::{AppCmd, EntryId, EntryKind, RenderData, View, ViewId};
    ...
    for index in (0..view1.len()).rev() {
        if view1.child(index).is::<Notification>() {
            ...
            if view2.rect() != view1.rect() {
                ...
                child.resize(*view2.rect(), &tx, rendering_ctx, context);
            }
...
pub fn toggle_main_menu(
    ...
    if let Some(index) = locate_by_id(view, ViewId::MainMenu) {
        ...
        RenderQueue::add_redraw_req(
            rendering_ctx,
            RenderData::expose(*view.child(index).rect(), UpdateMode::Gui),
        );
        ...
        RenderQueue::add_redraw_req(
            rendering_ctx,
            RenderData::new(main_menu.id(), *main_menu.rect(), UpdateMode::Gui),
        );
        view.children_mut()
...
pub fn toggle_battery_menu(
    ...
    if let Some(index) = locate_by_id(view, ViewId::BatteryMenu) {
        ...
        RenderQueue::add_redraw_req(
            rendering_ctx,
            RenderData::new(battery_menu.id(), *battery_menu.rect(), UpdateMode::Gui),
        );
        view.children_mut()
...
pub fn toggle_clock_menu(
    ...
    if let Some(index) = locate_by_id(view, ViewId::ClockMenu) {
        ...
        RenderQueue::add_redraw_req(
            rendering_ctx,
            RenderData::new(clock_menu.id(), *clock_menu.rect(), UpdateMode::Gui),
        );
        view.children_mut()
