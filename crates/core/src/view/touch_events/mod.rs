...
impl View for TouchEvents {
    fn handle_event(
        ...
        match evt {
            Event::Gesture(ge) => {
                let notif = Notification::new(ge.to_string(), None, hub, rendering_ctx, context);
    ...
    fn render_view(&self, rect: &Rectangle, ctx: &mut Context) {
        for x in rect.min.x..rect.max.x {
            for y in rect.min.y..rect.max.y {
                ...
                ctx.fb.set_pixel(x as u32, y as u32, color);
            }
        }
    }
    ...
    fn resize(
        ...
        self.children[0].resize(icon_rect, hub, rendering_ctx, context);
