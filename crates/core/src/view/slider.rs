...
use super::{Bus, Event, Hub, ID_FEEDER, Id, SliderId, THICKNESS_SMALL, View};
...
impl Slider {
    ...
    pub fn update(&mut self, value: f32, rendering_ctx: &mut Option<RenderQueue>) {
...
impl View for Slider {
        ...
        match *evt {
            Event::Device(DeviceEvent::Finger {
                ...
                FingerStatus::Motion if self.active && position.x != self.last_x => {
                    ...
                    RenderQueue::add_redraw_req(
                        rendering_ctx,
                        RenderData::no_wait(self.id, self.rect, UpdateMode::FastMono),
                    );
    ...
    fn render_view(&self, _rect: &Rectangle, ctx: &mut Context) {
        ...
        let font = font_from_style(&mut ctx.fonts, &SLIDER_VALUE, dpi);
        ...
        font.render(ctx.fb.as_mut(), PROGRESS_VALUE, &plan, pt);
    }
