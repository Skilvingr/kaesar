...
use crate::view::{Align, Bus, Event, Hub, ID_FEEDER, Id, View, ViewId};
...
impl TopBar {
    ...
    pub fn update_root_icon(&mut self, name: &str, rendering_ctx: &mut Option<RenderQueue>) {
    ...
    pub fn update_title_label(&mut self, title: &str, rendering_ctx: &mut Option<RenderQueue>) {
        let title_label = self.children[1].as_mut().downcast_mut::<Label>().unwrap();
        title_label.update(title, rendering_ctx);
    }

    pub fn update_frontlight_icon(
        &mut self,
    ...
    pub fn update_clock_label(&mut self, rendering_ctx: &mut Option<RenderQueue>) {
        if let Some(clock_label) = self.children[2].downcast_mut::<Clock>() {
            clock_label.update(rendering_ctx);
        }
    }
    ...
    pub fn update_battery_widget(
        ...
        if let Some(battery_widget) = self.children[3].downcast_mut::<Battery>() {
            battery_widget.update(capacity, status, rendering_ctx, context);
        }
    }
    ...
    pub fn reseed(
        ...
        self.update_frontlight_icon(rendering_ctx, context);
        self.update_clock_label(rendering_ctx);
        self.update_battery_widget(capacity, status, rendering_ctx, context);
    }
}
...
impl View for TopBar {
    fn handle_event(
        ...
        match *evt {
            ...
            Event::Gesture(GestureEvent::Movement { start, .. }) if self.rect.includes(start) => {
                true
            }
    ...
    fn resize(
        ...
        self.children[2].resize(clock_rect, hub, rendering_ctx, context);
